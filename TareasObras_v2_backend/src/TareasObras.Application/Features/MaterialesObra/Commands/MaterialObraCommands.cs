...
public record CreateMaterialObraCommand(Guid ObraId, string Descripcion, string Unidad, decimal Cantidad, decimal PrecioUnitario, DateTime Fecha, Guid? ProveedorId, string? NumeroAlbaran, string? NumeroFactura, string? Observaciones, Guid? LineaPartidaId) : IRequest<Guid>;
public record UpdateMaterialObraCommand(Guid Id, string Descripcion, string Unidad, decimal Cantidad, decimal PrecioUnitario, DateTime Fecha, Guid? ProveedorId, string? NumeroAlbaran, string? NumeroFactura, string? Observaciones, Guid? LineaPartidaId) : IRequest<bool>;
...
public class CreateMaterialObraHandler : IRequestHandler<CreateMaterialObraCommand, Guid>
{
    ...
    {
        var mat = MaterialObra.Create(r.ObraId, r.Descripcion, r.Unidad, r.Cantidad, r.PrecioUnitario, r.Fecha, r.ProveedorId, r.NumeroAlbaran, r.NumeroFactura, r.Observaciones, r.LineaPartidaId);
...
public class UpdateMaterialObraHandler : IRequestHandler<UpdateMaterialObraCommand, bool>
{
    ...
    public async Task<bool> Handle(UpdateMaterialObraCommand r, CancellationToken ct)
    {
        ...
        mat.Update(r.Descripcion, r.Unidad, r.Cantidad, r.PrecioUnitario, r.Fecha, r.ProveedorId, r.NumeroAlbaran, r.NumeroFactura, r.Observaciones, r.LineaPartidaId);
        await _uow.SaveChangesAsync(ct);
        return true;
    }
}
